...
class VideoInfoFrame:
    def __init__(self, master=None, font=("Segoe UI Black", 10)):
        ...
        self.search_data = lambda: self.youtube.thread_function(
            self.show_info, (self.url_entry.get(),)
        )
        self.download_video_function = lambda: self.youtube.thread_function(
            self.youtube.download_video,
            (self.url_entry.get(), self.path, self.progress_function),
        )
        ...
        self.thumbnail = ctk.CTkImage(
            Image.open(r"assets\Images\Logo.png"), size=(564, 247)
        )
        self.thumbnail_label = ctk.CTkLabel(
            self.thumbnail_frame, text="", image=self.thumbnail, anchor="center"
        )
        ...
        self.duration_label = ctk.CTkLabel(
            self.info_frame, text="Duration: ", font=font
        )
        ...
        self.filesize_label = ctk.CTkLabel(
            self.info_frame, text="Download Size: ", font=font
        )
        ...
        self.download_label = ctk.CTkLabel(
            self.download_progress_frame, text="Downloading...", font=font
        )
        self.download_label.grid(row=0, column=0, padx=10, pady=2.5, sticky="W")

        self.downloaded_percentage_label = ctk.CTkLabel(
            self.download_progress_frame, text="", font=font
        )
        ...
        self.download_button = ctk.CTkButton(
            ...
            width=50,
            font=font,
            command=self.download_video_function,
        )
    ...
    def show_info(self, url):
        try:
            ...
            thumbnail = wget.download(
                data_found["thumbnail-url"], "temp/thumbnail.png", bar=None
            )
            thumbnail = Image.open(thumbnail)
            ratio = thumbnail.size[0] / 300
            self.thumbnail = ctk.CTkImage(
                thumbnail, size=(thumbnail.size[0] / ratio, thumbnail.size[1] / ratio)
            )
            self.thumbnail_label.configure(
                self.info_frame, text="", image=self.thumbnail
            )
    ...
    def progress_function(self, stream, chunk, bytes_remaining):
        video_size = self.youtube.fromBytes(
            stream.filesize
        )  # Size of the video in prettified Format(from bytes to KB, MB, etc.)
        video_size_type = self.youtube.prettifyBytes(
            video_size
        )  # Video size format KB, MB, etc.
        bytes_remaining = stream.filesize - bytes_remaining  # Remaining download size
        percentage = self.youtube.toPercentage(
