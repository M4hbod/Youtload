...
from frames.AddUrlFrame.add_url_frame import AddUrlFrame
...
class MainApp:
    def __init__(self):
        ...
        self.add_url_frame = AddUrlFrame(self.root)

        # ==== Icons ====
        self.icon_size = 30
        self.addurl_icon = ctk.CTkImage(
            Image.open(r"assets\Icons\LightModeImages\addurl.png").resize(
                (self.icon_size, self.icon_size), Image.NEAREST
            )
        )
        self.download_icon = ctk.CTkImage(
            Image.open(r"assets\Icons\LightModeImages\download.png").resize(
                (self.icon_size, self.icon_size), Image.NEAREST
            )
        )
        self.settings_icon = ctk.CTkImage(
            Image.open(r"assets\Icons\LightModeImages\settings.png").resize(
                (self.icon_size, self.icon_size), Image.NEAREST
            )
        )
        ...
        self.app_name = ctk.CTkLabel(
            self.menu_frame, text="Youtload", font=self.tab_title
        )
        self.app_name.grid(row=1, column=0, padx=10, pady=10, sticky="nswe")

        # Buttons
        self.addurl_button = ctk.CTkButton(
            self.menu_frame,
            image=self.addurl_icon,
            compound="left",
            text="Add Url",
            corner_radius=15,
            width=40,
            font=self.text_font,
            command=lambda: self.toggleFrame("addurl"),
        )
        ...
        self.downloads_button = ctk.CTkButton(
            ...
            width=40,
            font=self.text_font,
            command=lambda: self.toggleFrame("downloads"),
        )
        ...
        self.settings_button = ctk.CTkButton(
            ...
            width=40,
            font=self.text_font,
            command=lambda: self.toggleFrame("settings"),
        )
